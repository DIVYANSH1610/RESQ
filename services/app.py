            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "- **Homepage (RESQ/homepage.html)**: Main page with an overview and SOS button to send emergency alerts.\n"
                        "- **Interactive Map (/map)**: Shows Quell Zones (safe areas like Civil Hospital, Lohia Hospital, Community Shelter "
                        "in Lucknow), hospitals, and SOS points with navigation and real-time weather.\n"
                        "- **SOS Page (/sos)**: Allows users to send emergency alerts and connect with volunteers via Q-Link.\n"
                        "- **AutoQ**: Predictive routing to guide users to the nearest Quell Zone (accessible via the map).\n"
                        "- **Q-Link**: Communication bridge for users, volunteers, and command centers (accessible via /sos).\n"
                        "When users ask about using the platform, guide them to the relevant page or feature with clear instructions, "
                        "including URLs (e.g., '/map' for the map) or actions (e.g., 'Click the SOS button'). Use HTML links in responses "
                        "like <a href=\"/map\" data-action=\"open_map\">Go to Map</a> for navigation. For platform-related queries, keep "
